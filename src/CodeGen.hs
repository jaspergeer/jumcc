...
visitExtDecl _ _ = error "fatal error: external declaration visit failed"
...
visitStat (AsmProg pname asm sim label) (ArrDeclS (VarDecl (Arr size typ) var) []) = AsmProg pname (asm ++ ["r2 := r2 + "] ++ ["r4 := r2"]
