...
visitExtDecl prog@(AsmProg asm sim label) (FuncDecl _ ident params body) = case stkSimPush (_sspa (stkSimPushFrame sim ident) params) (Var ".ret", CInt) of
...
visitStat prog (AssignS var exp) = case visitExpr prog exp of
...
visitExpr (AsmProg asm sim label) exp = case _vexpr (AsmProg asm (stkSimPushFrame sim ".expr") label) exp of
    (AsmProg asm1 sim1@(StackSim ((StkFrame _ (_:fs) _):_)) label1) -> AsmProg asm1 (stkSimPush (stkSimPopFrame sim1) (Var ".result", CInt)) label1

_vexpr :: AsmProg -> Expr -> AsmProg
-- stack-push operations --
_vexpr (AsmProg asm sim label) (IntE x) = AsmProg (asm ++ ["push " ++ show x ++" on stack r2"]) (stkSimPush sim (Var ".int", CInt)) label
_vexpr (AsmProg asm sim label) (CharE x) = AsmProg (asm ++ ["push " ++ show (ord x) ++ " on stack r2"]) (stkSimPush sim (Var ".char", CChar)) label
_vexpr (AsmProg asm sim label) (VarE var) = AsmProg (asm 
                                                    ++ ["r4 := m[r0][" ++ "r2 + " ++ show (stkSimQuery sim var) ++ "]"]
                                                    ++ ["push r4 on stack r2"]) (stkSimPush sim (Var ".var", CInt)) label
_vexpr (AsmProg asm sim label) In = AsmProg (asm 
                                                ++ ["r4 := input()"]
                                                ++ ["push r4 on stack r2"]) (stkSimPush sim (Var ".in", CInt)) label
